...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const blogURL = `http://localhost:8888/posts`
    let allBlogs = [] //storing book data
    
    //READ
    //fetching data and iterating data into UI
    //run forEach on blogData, which will run a function on each individual book in blogData
    fetch(`${blogURL}`)
      .then( response => response.json() ) //read and parse the data using json()
        .then( blogData => blogData.forEach(function(blog) {
        allBlogs = blogData  //empty array is now assigned to the blogData
        bookContainer.innerHTML += `
        <div id=book-${blog.id}>
            <h2>${blog.title}</h2>
            <h4>Author: ${blog.author}</h4>
            <p id="description">${blog.description}</p>
            <button data-id=${blog.id} id="edit-${blog.id}" data-action="edit">Edit</button>
            <button data-id=${blog.id} id="delete-${blog.id}" data-action="delete">Delete</button>
        </div>
        <div id=edit-book-${blog.id}>
        </div>`
    })) 
    ...
    if (e.target.dataset.action === 'edit') {
        ...
        const blogData = allBlogs.find((blog) => {
            return blog.id == e.target.dataset.id
        })
        ...
        editForm.innerHTML = `
            ...
            <form id="book-form">
                <input required id="edit-title" placeholder="${blogData.title}">
                <input required id="edit-author" placeholder="${blogData.author}">
                <input required id="edit-description" placeholder="${blogData.description}">
            ...
            const editedBook = document.querySelector(`#book-${blogData.id}`)

            fetch(`${blogURL}/${blogData.id}`, {
                method: 'PATCH',
                body: JSON.stringify({
                    title: titleInput,
                    author: authorInput,
        ...
        document.querySelector(`#book-${e.target.dataset.id}`).remove()
            fetch(`${blogURL}/${e.target.dataset.id}`, {
            method: 'DELETE',
            headers: {
                    'Content-Type': 'application/json'
                }
